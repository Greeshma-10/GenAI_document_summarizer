...
Endpoints:
  ...
  GET  /graph/subgraph           — N-hop neighbourhood (for visualisation)
...
def query_entity(
    ...
    if not results:
        raise HTTPException(status_code=404, detail=f"No relationships found for '{entity}'")
...
def query_path(
    ...
    Example: /graph/path?from=BLEU&to=Google+Brain
    """
    svc = get_service()
    ...
    if not result["found"]:
        raise HTTPException(
            status_code=404,
            detail=f"No path found between '{from_entity}' and '{to_entity}' within 6 hops"
        )
    return result
...
def query_by_type(
    ...
    entity_type must be one of: MODEL, DATASET, METRIC, ORGANIZATION, TASK, CONCEPT
    """
    valid_types = {"MODEL", "DATASET", "METRIC", "ORGANIZATION", "TASK", "CONCEPT"}
    if entity_type.upper() not in valid_types:
        raise HTTPException(
            status_code=400,
            detail=f"Invalid type '{entity_type}'. Must be one of: {', '.join(sorted(valid_types))}"
        )
    svc = get_service()
    results = svc.query_by_type(entity_type, limit)
    if not results:
        raise HTTPException(status_code=404, detail=f"No entities of type '{entity_type}' found")
...
def query_subgraph(
    ...
    Response is shaped for graph visualisation (nodes + edges lists).

    Example: /graph/subgraph?entity=Transformer&depth=2
    """
    svc = get_service()
...
# ─────────────────────────────────────────────────────────────────────────────
# 5. POST /graph/ask   { "question": "How is BLEU related to Transformer?" }
# ─────────────────────────────────────────────────────────────────────────────
@router.post("/ask", response_model=NLQueryResponse)
def query_natural_language(request: NLQueryRequest):
    """
    Natural language graph query — converts your question to Cypher and runs it.

    Example questions:
    - "Which models were developed by Google Brain?"
    - "How is BLEU related to the Transformer?"
    - "What datasets was the Transformer trained on?"
    - "Show me all concepts used in the Transformer"
    - "Which organizations proposed models?"
    """
